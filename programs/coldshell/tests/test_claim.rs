...
use {coldshell::constants::{DAY_SECONDS, RECORD_LATE_SECONDS}, common::*, solana_keypair::Keypair, solana_signer::Signer};
...
/// through Sunday, and — because a day keeps six hours past itself — finish Sunday's minute in
/// the small hours of Monday, which is also when the money comes back.
#[test]
fn a_stake_runs_the_following_week() {
    ...
    // Sunday's minute still goes in after midnight, on what is already Monday.
    ...
    // The week settles when its last day stops being recordable — six hours in — and there are
    // four weeks to come and collect it.
    let monday_morning = shell_start(first + 1) + RECORD_LATE_SECONDS - DAY_SECONDS;
    assert_eq!(monday_morning, settles(first, 0));
    set_time(&mut env.svm, monday_morning);
